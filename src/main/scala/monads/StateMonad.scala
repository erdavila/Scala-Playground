...
object StateMonad extends Monad2Companion {
  override protected type MM[Z, A] = StateMonad[Z, A]

  def `return`[S] = new Return[S]
  class Return[S] {
    def apply[A](a: A): MM[S, A] = { s => (a, s) }
  }
...
object StateMonadTest extends App {
  ...
  assert {
    val lift = StateMonad.liftM2((_: Int) + (_: Double))
